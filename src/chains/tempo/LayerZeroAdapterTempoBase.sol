...
import { BaseAdapter, IBridgeCoordinator } from "../../adapters/BaseAdapter.sol";
import { IBridgeAdapter } from "../../interfaces/IBridgeAdapter.sol";
import { BridgeTypes } from "../../adapters/BridgeTypes.sol";

/**
 * @title LayerZeroAdapterTempoBase
 * @notice Tempo-local copy of LayerZeroAdapter
 * @dev This stays behavior-identical to LayerZeroAdapter. The only change is that a few functions are marked
 * virtual so Tempo can override the amount conversion points.
 */
contract LayerZeroAdapterTempoBase is BaseAdapter, OApp, OAppOptionsType3 {
    /**
    ...
    /// @inheritdoc IBridgeAdapter
    function bridgeType() public pure virtual returns (uint16) {
        return BridgeTypes.LAYER_ZERO;
    }

    /**
     * @notice Sets the LayerZero endpoint id and peer for a remote chain
     * @param chainId Canonical chain id used by the coordinator
     * @param endpointId LayerZero endpoint id for the remote chain
     * @param remoteAdapter Adapter address registered on the remote chain, encoded as bytes32
     */
    function setRemoteEndpointConfig(uint256 chainId, uint32 endpointId, bytes32 remoteAdapter) external onlyOwner {
        require(chainId != 0, InvalidZeroAddress());
    ...
    /**
     * @notice Uses the two-step ownership flow from Ownable2Step
     */
