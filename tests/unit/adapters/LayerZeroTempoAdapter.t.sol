...
contract LayerZeroTempoAdapterTest is TestHelperOz5 {
    ...
    function setUp() public override {
        ...
        feeTokens[1] = address(feeToken);
        createEndpoints(2, LibraryType.UltraLightNode, feeTokens);
        ...
        feeToken.approve(address(l2Adapter), type(uint256).max);
        vm.stopPrank();
    ...
    function test_bridgeDispatchesOutboundPacket() public {
        ...
        uint256 tokenFee = l1Adapter.estimateBridgeFee(CHAIN_ID_L2, message, bridgeOptions);

        deal(address(feeToken), address(coordinator), tokenFee);
        vm.prank(address(coordinator));
        l1Adapter.bridge(CHAIN_ID_L2, remoteAdapterId, message, refundAddress, bridgeOptions, messageId, tokenFee);
    ...
    function test_bridgeRefundsExcessFee() public {
        ...
        uint256 tokenFee = l1Adapter.estimateBridgeFee(CHAIN_ID_L2, message, bridgeOptions);
        uint256 overpayAmount = tokenFee + 1 ether;

        deal(address(feeToken), address(coordinator), overpayAmount);
        vm.prank(address(coordinator));
        l1Adapter.bridge(CHAIN_ID_L2, remoteAdapterId, message, refundAddress, bridgeOptions, messageId, overpayAmount);

        assertEq(feeToken.balanceOf(refundAddress), 1 ether, "refund amount incorrect");
    }
    ...
    function test_bridgeRevertsWhenPeerMismatch() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(LayerZeroTempoAdapter.PeersMismatch.selector, badPeer, remoteAdapterId));
        vm.prank(address(coordinator));
        l1Adapter.bridge(CHAIN_ID_L2, remoteAdapterId, message, refundAddress, bridgeOptions, messageId, 0);
    ...
    function test_bridgeRevertsWhenEndpointUnset() public {
        ...
        vm.prank(address(coordinator));
        l1Adapter.bridge(unconfiguredChain, remoteAdapterId, message, refundAddress, bytes(""), messageId, 0);
    }
    ...
    function test_setRemoteEndpointConfigEmitsEvent() public {
        ...
        emit LayerZeroTempoAdapter.EndpointIdConfigured(newChainId, newEndpointId);
        vm.prank(owner);
        l1Adapter.setRemoteEndpointConfig(newChainId, newEndpointId, newRemoteAdapter);
    }
